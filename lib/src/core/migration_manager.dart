...
class MigrationManager {
  ...
  Future<void> createTables(Database db, List<Type> models) async {
    for (final modelType in models) {
      ...
      if (info == null) {
        print('Warning: Model $modelType not registered, skipping table creation');
        continue;
      }
  ...
  Future<void> _createTable(Database db, ModelInfo info) async {
    if (info.columns.isEmpty) {
      throw ArgumentError('Cannot create table ${info.tableName} with no columns');
    }
    ...
    if (columns.isEmpty) {
      throw ArgumentError('Cannot create table ${info.tableName} - no valid column definitions');
    }
    ...
    try {
      ...
      if (!exists) {
        throw StateError('Table ${info.tableName} was not created successfully');
      }
    } catch (e) {
      ...
      for (final entry in info.columns.entries) {
        ...
        if (!existingColumns.contains(columnName)) {
          try {
            ...
            final columnDef = _buildAlterColumnDefinition(columnName, columnInfo);
            await db.execute(
